...
function JobCard({ job, onDelete, onStatusUpdate ,onEdit , onResumeUpload }) {
  ...
  const styles = {                  // ← step 2: styles moved INSIDE function
    card: { backgroundColor: darkMode ? '#16213e' : '#fff', color: darkMode ? '#fff' : '#333', padding: '20px', borderRadius: '8px', marginBottom: '16px', boxShadow: '0 2px 6px rgba(0,0,0,0.1)' },
    ...
    role: { color: darkMode ? '#aaa' : '#666', fontSize: '14px', marginTop: '4px' },
    badge: { padding: '4px 12px', borderRadius: '20px', color: '#fff', fontSize: '12px', fontWeight: 'bold' },
    notes: { color: darkMode ? '#ccc' : '#555', fontSize: '14px', marginBottom: '8px' },
    date: { color: darkMode ? '#aaa' : '#999', fontSize: '12px', marginBottom: '12px' },
    actions: { display: 'flex', gap: '12px', alignItems: 'center' },
    select: { padding: '6px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '14px', backgroundColor: darkMode ? '#0f3460' : '#fff', color: darkMode ? '#fff' : '#333' },
    editBtn: { padding: '6px 14px', backgroundColor: '#4f46e5', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '14px' },
